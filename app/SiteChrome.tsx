...
import {PrivacyChoices} from './VisitorAnalytics';
...
export async function Header(){ const categories=await getCategories().catch(()=>[]); const announcement='B2B WHOLESALE ONLY · BULK PRODUCTION · PAN-INDIA DOOR-TO-DOOR DELIVERY'; return <>
  ...
  <header className="site-header">
    ...
    <Suspense fallback={<nav className="desktop-nav" aria-label="Main navigation"/>}><MegaMenu categories={categories.map(({slug,name,family,images})=>({slug,name,family,images:images.slice(0,1)}))}/></Suspense>
    <SearchBox/>
...
const columns=[
 {title:'Wholesale products',links:[['All categories','/products'],['Size & price calculator','/custom-size'],['Bakery packaging','/products?family=Bakery'],['Café & QSR','/products?family=Caf%C3%A9s%20%26%20Beverages'],['Courier boxes','/products?family=Courier'],['Healthcare files','/categories/hospital-files']]},
...
export function Footer(){ return <footer className="footer"><div className="footer-main"><div className="footer-links">{columns.map(c=><div key={c.title}><h3>{c.title}</h3>{c.links.map(([l,h])=><Link href={h} key={l}>{l}</Link>)}</div>)}</div><div className="footer-signup"><span>Wholesale enquiries</span><h2>Planning your next<br/>packaging run?</h2><p>Share dimensions, monthly volume, printing and destination. Our Jaipur team will reply with the right board and price tier.</p><Link className="footer-quote" href="/quote">Request a B2B quote →</Link><small>Door-to-door shipping available across India.</small></div></div><div className="footer-bottom"><Link className="wordmark footer-mark" href="/">Paper <i>&amp;</i> Press</Link><p>JAIPUR · PRINT · PACKAGING · POSSIBILITIES</p><div><PrivacyChoices/><span>GST invoices</span><span>Bulk only</span><span>Pan-India</span></div></div></footer>; }

export type Product={images?:string[];slug:string;name:string;eyebrow:string;delivery:string;price:string;image:string;className?:string;badge?:string};
export function ProductCard({product}:{product:Product}){return <Link className="product-card" href={`/products/${product.slug}`}><div className={`product-image ${product.className||''}`}><HoverCardGallery images={product.images?.length?product.images:productCardGallery(product.slug,product.image)} alt={product.name}/>{product.badge&&<span className="card-badge">{product.badge}</span>}<span className="product-arrow">↗</span></div><div className="product-meta"><p>{product.eyebrow}<span>{product.delivery}</span></p><h3>{product.name}</h3><small>{product.price}</small></div></Link>}
