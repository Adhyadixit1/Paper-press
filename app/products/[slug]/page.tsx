...
import {getProducts,getEntry} from '../../../lib/catalog';
...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
  ...
  const product=await getEntry('product',slug);
  if(!product)notFound();return pageSeo(product.seo_title||`${product.name} — Paper & Press`,product.seo_description||product.summary,`/products/${slug}`,product.images);
}
...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){
  ...
  const product=await getEntry('product',slug);
  if(!product)notFound();
  const gallery=product.images.length?product.images:productGalleryFor(slug);

  return <main className="product-detail"><ProductSchema name={product.name} description={product.summary} images={gallery} path={`/products/${slug}`}/>
    ...
    <section className="product-panel">
      ...
      {product.description&&<div style={{whiteSpace:'pre-line'}}>{product.description}</div>}<p>{product.summary}</p><strong>{product.price}</strong>
      ...
      <section className="configure-order" id="configure-order"><span>Build your wholesale order</span><h2>Choose size, material &amp; quantity.</h2><ProductConfigurator minimum={product.moq} slug={product.slug} name={product.name} price={product.price} sizes={product.sizes}/></section>
